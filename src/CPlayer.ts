...
export interface CPlayer {
	init(song: any): void;
	generate(): number;
	createAudioBuffer(context: any): any;
	createWave(): any;
	getData(t: number, n: number): number[];
}
export interface CPlayerConstructor {
	new(): CPlayer;
}

export const CPlayer: CPlayerConstructor = <any>function(this: CPlayer): any {
	...
	// Oscillators
	var osc_sin = function (value: number) {
		return Math.sin(value * 6.283184);
	};

	var osc_saw = function (value: number) {
		return 2 * (value % 1) - 1;
	};

	var osc_square = function (value: number) {
		return (value % 1) < 0.5 ? 1 : -1;
	};

	var osc_tri = function (value: number) {
		var v2 = (value % 1) * 4;
		if(v2 < 2) return v2 - 1;
		return 3 - v2;
	};

	var getnotefreq = function (n: number) {
		// 174.61.. / 44100 = 0.003959503758 (F3)
		return 0.003959503758 * (2 ** ((n - 128) / 12));
	};

	var createNote = function (instr: any, n: number, rowLen: number) {
		...
		// Local variables.
		var j, j2, e, t, rsample, o1t: any, o2t: any;
	...
	var mSong: any, mLastRow: any, mCurrentCol: any, mNumWords: any, mMixBuf: any;
	...
	this.createAudioBuffer = function(context: any) {
	...
	this.getData = function(t: number, n: number) {
